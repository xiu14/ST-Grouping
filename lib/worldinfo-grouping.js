...
function isWorldEditorVisible() {
    ...
    return isVisibleElement(worldPopup);
}
...
export function initWorldInfoGrouping() {
    ...
    worldPopupVisibilityObserver = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                if (isWorldEditorVisible()) {
                    if (worldEditorSessionActive) return;
                    worldEditorSessionActive = true;
                    onWorldEditorOpen();
                } else if (worldEditorSessionActive) {
    ...
    const startObserver = () => {
        ...
        if (worldPopup) {
            worldPopupVisibilityObserver?.observe(worldPopup, { attributes: true, attributeFilter: ['style'] });
            if (isWorldEditorVisible() && !worldEditorSessionActive) {
                worldEditorSessionActive = true;
                void onWorldEditorOpen();
            }
        }
    };
...
async function onWorldEditorOpen() {
    ...
    pushDebugLog('world-editor-open', { openCount: debugState.openCount });
...
function onWorldEditorClose() {
    console.log('[ST-Grouping] World editor closed');
    debugState.closeCount += 1;
    pushDebugLog('world-editor-close', { closeCount: debugState.closeCount });
