...
let hasLoadedCurrentWorldEntries = false;
...
async function refreshGroupedViewWhenEntriesReady(attempt = 0, maxAttempts = 8) {
    if (!hasLoadedCurrentWorldEntries) {
        scheduleRefreshGroupedViewWhenEntriesReady(120, attempt, maxAttempts);
        return;
    }
    ...
    if (expectedCount > 0 && renderedCount < expectedCount) {
        const nextAttempt = Math.min(attempt + 1, maxAttempts);
        const nextDelay = attempt < maxAttempts ? 120 : 300;
...
async function refreshGroupedView() {
    ...
    try {
        ...
        const expectedUngroupedUidSet = new Set(ungroupedUids.map(String));
        const misplacedUngroupedEntries = [];

        entriesList.find('.wi-group-container .world_entry').each(function () {
            const entryElement = $(this);
            const uid = entryElement.attr('uid');
            if (uid != null && expectedUngroupedUidSet.has(String(uid))) {
                misplacedUngroupedEntries.push(entryElement);
            }
        });

        if (misplacedUngroupedEntries.length) {
            for (const entryElement of misplacedUngroupedEntries) {
                entriesList.append(entryElement.detach());
            }
        }

        let renderedUngroupedEntries = entriesList.children('.world_entry');
        ...
        if (hasUngroupedMismatch) {
            const renderedUngroupedUids = new Set();
            renderedUngroupedEntries.each(function () {
                const uid = $(this).attr('uid');
                if (uid != null) {
                    renderedUngroupedUids.add(String(uid));
                }
            });

            const missingUngroupedUids = ungroupedUids.filter(uid => !renderedUngroupedUids.has(String(uid)));
            const misplacedUngroupedUids = [];
            const absentUngroupedUids = [];

            for (const uid of missingUngroupedUids) {
                const entryElement = entriesList.find(`.world_entry[uid="${uid}"]`).first();
                if (entryElement.length) {
                    misplacedUngroupedUids.push(uid);
                } else {
                    absentUngroupedUids.push(uid);
                }
            }
            ...
            if (misplacedUngroupedUids.length > 0) {
                renderedUngroupedEntries = entriesList.children('.world_entry');
                renderedUngroupedCount = renderedUngroupedEntries.length;
                hasUngroupedMismatch = renderedUngroupedCount !== expectedUngroupedCount;
                ungroupedCountText = hasUngroupedMismatch
                    ? `${renderedUngroupedCount}/${expectedUngroupedCount}`
                    : String(expectedUngroupedCount);
            }
        }
...
export function initWorldInfoGrouping() {
    ...
    eventSource.on(event_types.WORLDINFO_ENTRIES_LOADED, async () => {
        ...
        if (currentWorldName) {
            hasLoadedCurrentWorldEntries = true;
    ...
    $(document).on('change', '#world_editor_select', async function () {
        ...
        if (selectedIndex && selectedText) {
            ...
            hasLoadedCurrentWorldEntries = false;
