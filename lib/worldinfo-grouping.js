...
function ensureContextMenuInteractionGuard() {
    if (contextMenuInteractionGuardBound) return;
    ...
    document.addEventListener('touchend', guardHandler, true);
...
function initLongPressMenu() {
    const entriesList = $('#world_popup_entries_list');
    if (!entriesList.length) return;
    ensureContextMenuInteractionGuard();
