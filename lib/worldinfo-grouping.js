...
function updateGroupedWorldEntryCounts(scope = document) {
    const $scope = $(scope);
    const $containers = $scope.hasClass('wi-group-container')
        ? $scope
        : $scope.find('.wi-group-container');

    $containers.each(function () {
        const $container = $(this);
        const $entries = $container.find('.wi-group-content .world_entry');
        ...
        $container.find('.wi-group-count').text(formatGroupCountText(enabledCount, totalCount));
    });
}
