...
function injectGlobalWorldSelector() {
    ...
    const $selector = $(`
        <div id="wi-global-world-selector" class="wi-global-world-selector">
            ...
            <div class="wi-gws-selected-books"></div>
        </div>
    `);
 ...
 * 更新已选世界书快捷编辑入口
 */
function updateSelectedWorldBooks() {
    const $selectedBooks = $('#wi-global-world-selector .wi-gws-selected-books');
    if (!$selectedBooks.length) return;

    const selectedValues = getSelectedWorldInfo();
    const selectedValueSet = new Set(selectedValues.map(value => String(value)));
    const selectedBooks = getAllWorldBooks().filter(book => selectedValueSet.has(String(book.value)));
    const editorValue = String($('#world_editor_select').val() ?? '');
    const editorName = String($('#world_editor_select option:selected').text() ?? '');

    $selectedBooks.empty();
    selectedBooks.forEach(book => {
        const isEditing = String(book.value) === editorValue || String(book.name) === editorName;
        const $book = $(`
            <div class="wi-gws-selected-book ${isEditing ? 'active' : ''}" data-value="${escapeHtml(book.value)}" data-name="${escapeHtml(book.name)}" title="编辑 ${escapeHtml(book.name)}" role="button" tabindex="0">
                <i class="fa-solid fa-pen-to-square wi-gws-selected-book-icon"></i>
                <span class="wi-gws-selected-book-name">${escapeHtml(book.name)}</span>
            </div>
        `);
 ...
 * 打开对应世界书编辑器
 */
function openSelectedGlobalWorldBook(value, name = '') {
    const $editorSelect = $('#world_editor_select');
    if (!$editorSelect.length) return;

    const targetValue = String(value ?? '');
    let $targetOption = $editorSelect.find('option').filter(function () {
        return String($(this).val()) === targetValue;
    }).first();

    if (!$targetOption.length && name) {
        $targetOption = $editorSelect.find('option').filter(function () {
            return String($(this).text()) === String(name);
        }).first();
    }

    if (!$targetOption.length) {
        console.warn('[ST-Grouping] Failed to open global world book, option not found:', targetValue, name);
        return;
    }

    const editorValue = String($targetOption.val() ?? '');
    if (String($editorSelect.val() ?? '') !== editorValue) {
        $editorSelect.val(editorValue).trigger('change');
    }
...
function bindGlobalWorldSelectorEvents($selector) {
    ...
    // 点击已选世界书，直接打开编辑
    $selector.on('click', '.wi-gws-selected-book', function (e) {
        e.stopPropagation();
        openSelectedGlobalWorldBook($(this).attr('data-value'), $(this).attr('data-name'));
    });

    $selector.on('keydown', '.wi-gws-selected-book', function (e) {
    ...
    $(document).off('change.wigws', '#world_info').on('change.wigws', '#world_info', function () {
        updateGlobalWorldSelectorContent();
    });

    $(document).off('change.wigws', '#world_editor_select').on('change.wigws', '#world_editor_select', function () {
        updateSelectedWorldBooks();
    });
}
